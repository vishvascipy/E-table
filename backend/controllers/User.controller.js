...
module.exports = class User {
    ...
    static apiSignInUser = async (req, res) => {
        try {
            ...
            console.log(userSignedIn.admin);

            return res.status(201).json({ "jwt_token": userSignedIn.token, "admin": userSignedIn.admin });
    ...
    static apiGetAuth = async (req, res) => {
        res.json({ auth: true, userid: req.user.user_id })
    }
}